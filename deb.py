...
while 1:
        ...
        archivePage = "Vikipedi:Devriyelik_başvurusu/Arşiv/" + str(currentMonth) + "_" + str(currentYear)
        now = datetime.now()
        content = mavri.content_of_section(wiki, title, section, xx)

        if content != '':
            timestamp = re.findall('\{\{\s*User:Evrifaessa\/DEB\s*\|\s*([^\|\}]*)\s*\|\s*[^\|\}]*\s*\}\}', content)
            informer = re.findall('\{\{\s*User:Evrifaessa\/DEB\s*\|\s*[^\|\}]*\s*\|\s*([^\|\}]*)\s*\}\}', content)
            if timestamp and informer:
                ...
                if resolved:
                    ...
                    print('Başvuru henüz sonuçlandırılmamış, arşivlenmiyor.')
            section += 1
        else:
            section = 1
        time.sleep(15)